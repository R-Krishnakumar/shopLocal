...
def update():
    ...
    with open("product.csv", 'r') as csvfile:
        ...
        for row in csvreader:
            index +=1
            db.execute("INSERT INTO products (product_id, product_name, description, unit, unit_cost) VALUES(:id, :product_name, :description, :unit, :unit_cost)", id = row[0], product_name = row[1], description = row[2], unit = row[3], unit_cost = row[4])
    return homepage()
...
def homepage():
    if request.method=="GET":
        ...
        count = db.execute("SELECT COUNT(*) FROM products")[0]["COUNT(*)"]
        for i in range(1, count+1):
            if request.form.get("rate"+str(i)) != "" and request.form.get("rate"+str(i)) != None:
                db.execute("UPDATE products SET unit_cost = :unit_cost WHERE product_id=:product_id", unit_cost = request.form.get("rate"+str(i)), product_id = i)
        if request.form.get("new_product")!="" and request.form.get("new_description")!="" and request.form.get("new_unit")!="" and request.form.get("new_unit_cost")!="":
            product_name = request.form.get("new_product").replace(" ", "_")
            print(type(product_name))
            description = request.form.get("new_description")
            unit = request.form.get("new_unit")
            unit_cost = request.form.get("new_unit_cost")
            db.execute("INSERT INTO products (product_name, description, unit, unit_cost) values (:product_name, :description, :unit, :unit_cost)", product_name = product_name, description = description, unit = unit, unit_cost = unit_cost)
        rows = db.execute("SELECT * FROM products")
        credit_limit = db.execute("SELECT * FROM users WHERE user_id = :user_id", user_id=session["user_id"])[0]["credit_limit"]
        return render_template("homepage.html", rows=rows, username=session['username'], credit_limit=credit_limit)
...
def history():
    ...
    # else:
        ...
        for row in rows:
            total_value += row["value"]
            user_id=row["user_id"]
            row["username"] = db.execute("SELECT * FROM users WHERE user_id=:user_id", user_id=user_id)[0]["username"]
        return render_template("history.html", rows=rows, products=products, users=users, total_value=total_value, username=session["username"], message=message)
...
def orderbook():
    if request.method == "GET":
        ...
        return render_template("orderbook.html", rows=rows, username=session["username"])
    else:
        i = 1
        count = db.execute("SELECT COUNT(*) FROM transactions WHERE status =:status", status="To be delivered")[0]["COUNT(*)"]
        print(count)
        while (i <= count):
            print(request.form.get("status"+str(i)), i)
            tran_id = request.form.get("transaction_id"+str(i))
...
def credit_limit():
    if request.method == "GET":
        ...
        return render_template("credit_limit.html", rows=rows, username=session["username"])
    else:
        i = 1
        count = db.execute("SELECT COUNT(*) FROM users")[0]["COUNT(*)"]
        while (i <= count):
            if (request.form.get("amount"+str(i)) != ""):
                db.execute("UPDATE users SET credit_limit = credit_limit + :amount WHERE user_id=:user_id", amount = int(request.form.get("amount"+str(i))), user_id = request.form.get("user_id"+str(i)))
                db.execute('INSERT INTO transactions (product_name, unit, unit_cost, value, quantity, date, user_id, status) VALUES (:product_name, :unit, :unit_cost, :value, :quantity, :date, :user_id, :status)', product_name="Credit", unit="", unit_cost="", value = int(request.form.get("amount"+str(i))), quantity="", date=(datetime.today()).strftime('%Y-%m-%d'), user_id = request.form.get("user_id"+str(i)), status="Credited")
            i +=1
        rows = db.execute("SELECT * FROM users")
        return render_template("credit_limit.html", rows=rows, username=session["username"])
...
def cartload():
    if session["user_id"]!=3:
        rows = db.execute("SELECT * FROM cart WHERE user_id=:user_id", user_id = session["user_id"])
        total_value=db.execute("SELECT SUM(value) FROM cart WHERE user_id=:user_id", user_id = session["user_id"])[0]["SUM(value)"]
    else:
        rows = db.execute("SELECT * FROM cart")
        total_value=db.execute("SELECT SUM(value) FROM cart")[0]["SUM(value)"]
    credit_limit = db.execute("SELECT * FROM users WHERE user_id = :user_id", user_id=session["user_id"])[0]["credit_limit"]
    if len(rows)>0:
        return render_template("cart.html", rows = rows, total_value=total_value, user_id = session["user_id"], credit_limit = credit_limit, username=session["username"])
    else:
        emptymessage = "Cart is empty"
        return render_template("cart.html", rows = rows, total_value=total_value, user_id = session["user_id"], message1 = emptymessage, credit_limit = credit_limit, username=session["username"])
